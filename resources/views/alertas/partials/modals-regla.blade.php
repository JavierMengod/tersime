...
<div class="modal fade" id="modal-rule-create" tabindex="-1" aria-labelledby="modal-rule-create-label" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
    <form method="POST" action="{{ route('alertas.reglas.store') }}" class="modal-content">
      @csrf

      <div class="modal-header border-0 pb-0">
        <div>
          <h5 class="modal-title fw-bold" id="modal-rule-create-label">{{ __('Nueva regla de alerta') }}</h5>
          <p class="text-muted small mb-0">{{ __('Define cuándo y cómo quieres ser notificado') }}</p>
        </div>
      ...
      <div class="modal-body pt-3">
              ...
              @foreach ($dispositivos as $d)
                <div class="form-check form-check-hoverable py-1 px-3 rounded">
                  <input class="form-check-input" type="checkbox"
                         name="devices[]" value="{{ $d->id }}"
                         id="create-dev-{{ $d->id }}"
                         {{ in_array($d->id, old('devices', [])) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
          ...
          <div class="d-flex flex-wrap gap-2 mb-3">
            <div>
              <input type="checkbox" class="btn-check" id="create-ch-telegram"
                     name="methods[]" value="telegram" autocomplete="off"
                     {{ in_array('telegram', old('methods', [])) ? 'checked' : '' }}>
              <label class="btn btn-outline-info d-flex align-items-center gap-2 px-3" for="create-ch-telegram">
                <i class="fab fa-telegram"></i> Telegram
              </label>
            </div>
            <div>
              <input type="checkbox" class="btn-check" id="create-ch-email"
                     name="methods[]" value="email" autocomplete="off"
                     {{ in_array('email', old('methods', [])) ? 'checked' : '' }}>
              <label class="btn btn-outline-warning d-flex align-items-center gap-2 px-3" for="create-ch-email">
                <i class="fas fa-envelope"></i> {{ __('Correo') }}
              </label>
            </div>
            <div>
              <input type="checkbox" class="btn-check" id="create-ch-discord"
                     name="methods[]" value="discord" autocomplete="off"
                     {{ in_array('discord', old('methods', [])) ? 'checked' : '' }}>
              <label class="btn btn-outline-secondary d-flex align-items-center gap-2 px-3" for="create-ch-discord">
                <i class="fab fa-discord"></i> Discord
              </label>
            </div>
          </div>

          <div id="create-tpl-telegram" style="{{ in_array('telegram', old('methods', [])) ? '' : 'display:none' }}">
            <div class="border rounded p-3 mb-2" style="border-color:var(--bs-info)!important;border-left-width:3px!important;">
              ...
              <textarea name="template_telegram" class="form-control form-control-sm" rows="2"
                        placeholder="{{ __('Ej: 🚨 Atención: consumo elevado en {dispositivo}') }}">{{ old('template_telegram') }}</textarea>
            </div>
          </div>

          <div id="create-tpl-email" style="{{ in_array('email', old('methods', [])) ? '' : 'display:none' }}">
            <div class="border rounded p-3 mb-2" style="border-color:var(--bs-warning)!important;border-left-width:3px!important;">
              ...
              <textarea name="template_email" class="form-control form-control-sm mb-2" rows="2"
                        placeholder="{{ __('Cuerpo del correo...') }}">{{ old('template_email') }}</textarea>
          ...
          <div id="create-tpl-discord" style="{{ in_array('discord', old('methods', [])) ? '' : 'display:none' }}">
            <div class="border rounded p-3 mb-2" style="border-color:var(--bs-secondary)!important;border-left-width:3px!important;">
              ...
              <textarea name="template_discord" class="form-control form-control-sm" rows="2"
                        placeholder="{{ __('Mensaje para el webhook de Discord...') }}">{{ old('template_discord') }}</textarea>
            </div>
          </div>
        </div>
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
    <form id="form-rule-edit" method="POST" class="modal-content">
      ...
      <div class="modal-footer border-0 pt-0 justify-content-between">
        <button type="button" id="edit-btn-delete" class="btn btn-outline-danger btn-sm">
